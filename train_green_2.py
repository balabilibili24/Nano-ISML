...
from dataset.dataset_green_2 import dataset_green_2
...
def train(**kwargs):
    ...
    # data
    train_data = dataset_green_2(opt.train_data_root, train=True)
    val_data = dataset_green_2(opt.train_data_root, val=True)
...
def test(**kwargs):
    ...
    # data
    test_data = dataset_green_2(opt.test_data_root, test=True)
    ...
    for ii, (test_input, img_path_green) in tqdm(enumerate(test_dataloader)):
        ...
        import cv2
        cv2.imwrite(img_path_green[0].split("/Image")[0]+"/predict_2.1.png", seg_mask)
