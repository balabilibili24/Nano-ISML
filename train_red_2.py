...
from dataset.dataset_red_2 import dataset_red_2
...
def train(**kwargs):
    ...
    # data
    train_data = dataset_red_2(opt.train_data_root, train=True)
    val_data = dataset_red_2(opt.train_data_root, val=True)
...
def test(**kwargs):
    ...
    # data
    test_data = dataset_red_2(opt.test_data_root, test=True)
    ...
    for ii, (test_input, img_path_red) in tqdm(enumerate(test_dataloader)):
        ...
        import cv2
        cv2.imwrite(img_path_red[0].split("/Image")[0]+"/predict_1.1.png", seg_mask)

        #seg_dice = seg_criterion(seg_score, mask)

        #print(1 - seg_dice.item())
